...
drawcon draw_mod (.SW(SW), .xData(yData), .yData(xData), .pixclk(pixclk), .frameclk(frameclk), .ctrBtn(ctrBtn), .draw_r(tmpR), .draw_g(tmpG), .draw_b(tmpB), .draw_x(curr_x), .draw_y(curr_y));
